...
def _find_text_token_span(text: str) -> Tuple[int, int, torch.Tensor]:
    ...
    if char_start == -1:
        raise ValueError(
            ...
            "Check that QUESTION/SYSTEM_PROMPT don't contain the same substring."
        )
...
def whitebox_attack(
    ...
    print(f"\nOriginal  log-odds={base_r:+.3f} ('{QUESTION}': YES if >0, NO if <0)  ppl={orig_ppl:.1f}")
    print(start_text)
